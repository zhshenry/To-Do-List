...
export function AssistantApp({ compact = false, embedded = false, closing = false, back, expand, onClose, initialPrompt = null, consumedPrompt }: {
  ...
  const [answerPopPos, setAnswerPopPos] = useState<{ left: number; top: number } | null>(null);
  ...
  if (compact) {
    ...
    const plainAnswer = latestAssistant?.content ? markdownToPlainText(latestAssistant.content) : '';
    const openAnswerPop = () => {
      ...
      answerHoverTimer.current = window.setTimeout(() => {
        setAnswerPopPos({ left: rect.left, top: Math.min(rect.bottom + 6, window.innerHeight - 200) });
        setAnswerHover(true);
      }, 300);
    };
  ...
  {answerHover && answerPopPos && latestAssistant.content ? createPortal(
    <div className="hover-pop md-body" style={{ position: 'fixed', left: Math.max(8, Math.min(answerPopPos.left, window.innerWidth - 404)), top: answerPopPos.top, width: Math.min(396, window.innerWidth - 24) }} role="dialog" aria-label="AI 回复全文" onMouseEnter={cancelAnswerPopClose} onMouseLeave={scheduleAnswerPopClose}>
