...
export function MiniCard({ data, today, api, mode, setMode, draft, setDraft, mutating, motion, error, clearError, mutate, expand, edit }: {
  ...
  const [deck, setDeck] = useState<{ dir: 'next' | 'prev'; outgoing: Task; from: number; fromLen: number } | null>(null);
  const titleInput = useRef<HTMLInputElement>(null);
  const armTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
  const deckTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
  const previousCurrent = useRef<Task | null>(null);
  ...
  const insightTask = remaining.find(task => task.id === storedInsight?.taskId);
  const aiInsight = aiAvailable && storedInsight?.source === 'ai' && insightTask?.title === storedInsight.title && insightTask.dueAt === storedInsight.dueAt ? storedInsight : null;
  ...
  function page(delta: 1 | -1) {
    if (remaining.length < 2 || deck) return;
    if (armed) disarmArm();
    setDeck({ dir: delta > 0 ? 'next' : 'prev', outgoing: current, from: index, fromLen: remaining.length });
    setIndex(value => (value + delta + remaining.length) % remaining.length);
    if (deckTimer.current) clearTimeout(deckTimer.current);
    deckTimer.current = setTimeout(() => setDeck(null), 280);
  }
  useEffect(() => () => { if (deckTimer.current) clearTimeout(deckTimer.current); }, []);
  useEffect(() => {
    const prev = previousCurrent.current;
    previousCurrent.current = current ?? null;
    if (!current || !prev || prev.id === current.id || remaining.some(task => task.id === prev.id)) return;
    setDeck({ dir: 'next', outgoing: prev, from: index, fromLen: remaining.length + 1 });
    if (deckTimer.current) clearTimeout(deckTimer.current);
    deckTimer.current = setTimeout(() => setDeck(null), 280);
  }, [current, remaining]);
  ...
  function deckCard(task: Task, counter: string) {
    ...
    return <article className="mini-task-front">
      ...
      <div className="front-meta">
        ...
        {isArmed ? <span className="confirm-bar"><button type="button" className="confirm-btn" disabled={mutating} onClick={confirmComplete}>确认完成</button><button type="button" className="confirm-cancel" disabled={mutating} onClick={disarmArm}>取消</button></span> : <div className="mini-deck-pager"><span>{counter}</span><button type="button" aria-label="上一项" disabled={remaining.length < 2} onClick={() => page(-1)}><CaretDown size={9} className="is-up" /></button><button type="button" aria-label="下一项" disabled={remaining.length < 2} onClick={() => page(1)}><CaretDown size={9} /></button></div>}
      </div>
    </article>;
  }
  const titlebar = <>
    ...
    <section className="mini-window">
      ...
      <div className={`mini-content motion-${contentMotion || 'idle'}${mode === 'ai' ? ' is-ai' : ''}`}>
        {mode === 'home' ? suggestionOpen && suggestion ? <>
          ...
          {!aiAvailable ? !!data.settings.endpoint.trim() && !!data.settings.model.trim() ? <button type="button" className="mini-insight" aria-label="建议：配置 AI 后可获得个性化建议，启用 AI" disabled={mutating} onClick={() => void mutate(() => api.settings({ aiEnabled: true, autoStart: data.settings.autoStart }))}><span>建议：</span><b>配置 AI 后可获得个性化建议</b><em>启用</em></button> : <button type="button" className="mini-insight" aria-label="建议：配置 AI 后可获得个性化建议，打开 AI 配置" onClick={() => void api.openSettings(!(window.matchMedia?.('(prefers-reduced-motion: reduce)').matches ?? false))}><span>建议：</span><b>配置 AI 后可获得个性化建议</b><em>配置</em></button> : suggestion ? <button type="button" className={`mini-insight${insightIsAi ? ' is-ai' : ''}`} aria-expanded="false" aria-label={insightIsAi ? 'AI 生成的建议' : '建议'} onClick={() => setSuggestionOpen(true)}><span>{insightIsAi ? '✦ AI 建议' : '建议'}</span><b>{suggestion.title}</b><small>{renderInsightContext(suggestion.context, insightMinutes)}</small><em>查看</em></button> : null}
