// 真机证据（#16 v2）：确认按钮去图标 + 15px + 底行固定 16px 的注入演示。
// 运行时样式注入仅作用于本次截图会话，不改任何仓库代码。worktree 根目录运行：
// `node tooling/capture-16-v2.mjs <输出目录>`
...
// v2 样式注入（等价于实现改动）
await page.addStyleTag({ content: '.mini-home .front-meta { height: 16px; } .mini-home-with-insight .front-meta { height: 16px; } .confirm-btn, .confirm-cancel { min-height: 15px !important; } .confirm-btn svg { display: none; }' });
await page.waitForTimeout(150);

const normalV2 = await measure();
await page.locator('.mini-task-check').first().click();
await page.locator('.confirm-bar').waitFor();
await page.waitForTimeout(200);
const armedV2 = await measure();
await page.screenshot({ path: path.join(outputDir, '16-v2-armed.png') });

// armed 前后对比（v2 下）
await page.locator('.confirm-cancel').click();
await page.waitForTimeout(250);
const normalAfter = await measure();
await page.screenshot({ path: path.join(outputDir, '16-v2-normal.png') });
console.log('#16 v2 normal:', JSON.stringify(normalV2));
console.log('#16 v2 armed:', JSON.stringify(armedV2));
console.log('#16 v2 位移:', Math.abs(normalV2.titleTop - armedV2.titleTop).toFixed(1), 'px（normal 复测位移', Math.abs(normalAfter.titleTop - armedV2.titleTop).toFixed(1), 'px）');
await app.close();
console.log('saved 16-v2-armed.png / 16-v2-normal.png');
