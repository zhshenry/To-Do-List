...
try {
  ...
  assert.equal(await main.locator('.meta-progress').count(), 0, 'a card without maintained progress shows no percentage');
  await main.evaluate(async () => {
    const task = (await window.desktop.state()).tasks.find(item => item.title === '完成项目报告');
    await window.desktop.update(task.id, { progress: 0 }, task.updatedAt);
  });
  await main.getByRole('button', { name: '下一项', exact: true }).click();
  await poll(async () => await main.locator('.mini-task-stack.is-rolling').count() === 0, 'deck roll finishes');
  ...
  assert.equal((await main.evaluate(() => window.desktop.state())).tasks.find(task => task.title === '完成项目报告').status, 'todo', 'second click on the circle cancels');
  await main.getByRole('button', { name: '完成 完成项目报告', exact: true }).click();
  await main.getByRole('button', { name: '确认完成', exact: true }).click();
  assert.equal((await main.evaluate(() => window.desktop.state())).tasks.find(task => task.title === '完成项目报告').status, 'done');
  await poll(async () => await main.locator('.mini-task-stack.is-rolling').count() === 0, 'completed card leaves the deck');
  assert.equal(await main.locator('.mini-stack-sheet.middle').count(), 0, 'two cards do not draw a third sheet');
  const pairTop = parseFloat(await main.locator('.mini-stack-sheet.back').evaluate(element => getComputedStyle(element).top));
  assert.ok(pairTop < 11, `two-card backing sheet sits in the near slot, got ${pairTop}`);
