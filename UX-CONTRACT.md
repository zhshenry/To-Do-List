...
| 场景 | UI 行为 | 验证来源 |
| --- | --- | --- |
| 今日布局与复盘 | 空计划卡片紧凑呈现；明天/后天共用后续事项卡片。复盘从今日计划旁更多操作打开，关闭后焦点回到入口；底部只保留 AI 与提醒/反馈 | App.tsx / ux-smoke.mjs |
...
| 主窗口 AI 入口 | 底部星形按钮只打开 AI 悬浮对话；手动记录用日期右侧「新增」或 Ctrl+N，输入标题 Enter 保存 | App.tsx / ux-smoke.mjs |
...
| 今日 | 未完成事项跨日可见，不改截止时间；日期旁「事项库」/Ctrl+F 进入所有日期，支持搜索/标签筛选、未完成/已完成/已删除、恢复和批量完成/恢复 | TaskLibrary.tsx / ux-smoke.mjs |
...
| AI 对话 | 星形入口打开独立、非模态悬浮窗，主清单始终保留；主动发送才访问当前选用的模型服务；携带当前对话最近6轮并刷新事项快照；回复在普通对话内逐字流出，真实工具调用可折叠查看；可追问和解释。每项事项或标签变更各用一张可选择、卡内编辑的建议卡；待确认时仍可追问，后续方案替换并标记上一版；仅「应用所选」事务写入，放弃不写入；错误和取消不丢输入 | ai.test.ts / ux-smoke.mjs |
...
| 小卡片首页与新增 | 首页显示今天及逾期的未完成事项，有时间者优先并按时间、创建顺序排列；叠层只保留「新增事项」「AI 助手」两个操作。可执行 AI 建议最多一条，点击后在卡片内向下展开；未配置可用模型时仍保留建议栏，显示「请先配置AI大模型」并打开 AI 配置页。新增可切换待办/日程；时间、日期与提醒合一，时间为一列30分钟档位；优先级与标签纵向窄面板；展开态与小卡片的新建标签共用两行共16种预设色及相同选中态，确认后工具图标填充对应颜色；更多设置默认隐藏进度条，启用维护后显示，并可填写备注。同一工具按钮再次点击收起面板 | App.tsx / TaskEditor.tsx / MiniCard.tsx / ux-smoke.mjs / mini-card-test.mjs |
| 小卡片 AI | 原位横向切换并使用暖色边框追光；快捷指令随当前事项变化。右下角模型菜单向下展开并按供应商分组，同一按钮再次点击收起。Pi Agent Loop 不预估总步骤，只显示当前真实工具、已完成数量和不定进度；回复流式显示，处理记录只含真实工具事件。涉及事项或标签的建议必须点击「应用 N 项」才写入，放弃不写入 | AssistantApp.tsx / AIConversation.tsx / ai.test.ts / mini-card-test.mjs |
| AI 悬浮窗 | 对话气泡外形，从主界面打开时箭头指向右下角 AI 按钮；可拖动，在320×420至620×820间缩放。打开/关闭约180ms沿箭头方向淡入滑出；关闭仅隐藏且保留本次会话；主窗隐藏不隐藏 AI；已打开时再次调用仅聚焦；手动移动后停止跟随，隐藏后重新打开才重新定位。模型选择位于底部输入框内，标题栏只保留身份、状态、新对话和关闭 | main.ts / ux-smoke.mjs |
...
独立窗口启动：新安装只按保存状态显示主界面；悬浮入口即使保存为开启也不创建窗口，原开关、图片、位置和置顶配置不改写。`--hidden` 只抑制主窗，不覆盖其手动启动偏好。AI 从主界面或托盘打开，屏幕移除后约束回可用区域。当前验收由 `tooling/ux-smoke.mjs` 与 `tooling/mini-card-test.mjs` 确认 Dock 窗口不存在；`tooling/dock-regression.mjs` 保留给未来恢复功能时使用。
