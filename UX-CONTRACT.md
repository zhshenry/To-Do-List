...
| 场景 | UI 行为 | 验证来源 |
| --- | --- | --- |
| 今日布局与复盘 | 待办卡空时紧凑呈现；日程与后续日程同一张卡，今日列表始终可见，明天/近期/更晚折叠。复盘从工具栏更多操作打开，关闭后焦点回到入口；底部只保留 AI 与提醒/反馈 | App.tsx / ux-smoke.mjs |
...
| 今日 | 未完成待办不论完成期限是否在未来都留在待办卡，不改原截止日期；未完成日程按发生日，今日及逾期留在日程卡上部，更晚的进同卡折叠分组。日期旁「事项库」/Ctrl+F 进入所有日期，支持搜索/标签筛选、未完成/已完成/已删除、恢复和批量完成/恢复 | TaskLibrary.tsx / ux-smoke.mjs |
...
| 小卡片首页与新增 | 首页显示未完成待办（含未到期长任务）以及今日与逾期的未完成日程，有时间者优先并按时间、创建顺序排列。卡片左上角固定类型徽章（待办＝绿、日程＝蓝灰）与超期徽章（已超期＝红，右上角时间同转红），事项标题最多两行后省略，截断时悬浮显示完整标题。待办勾选为二次确认：首次点击进入待确认态（勾选框填充，底部就地出现 [✓ 确认完成] [取消]），再次点击才写入并翻到下一项；Esc/取消/翻页/4 秒无操作解除，无停留无撤销。叠层只保留「新增事项」「AI 助手」两个操作。可执行 AI 建议最多一条，点击后在卡片内向下展开；未配置可用模型时仍保留建议栏，显示「请先配置AI大模型」并打开 AI 配置页。新增可切换待办/日程；时间、日期与提醒合一，时间为一列30分钟档位；优先级与标签纵向窄面板；展开态与小卡片的新建标签共用两行共16种预设色及相同选中态，确认后工具图标填充对应颜色；更多设置默认隐藏进度条，启用维护后显示，并可填写备注。同一工具按钮再次点击收起面板 | App.tsx / TaskEditor.tsx / MiniCard.tsx / ux-smoke.mjs / mini-card-test.mjs |
