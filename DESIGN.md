...
使用 Windows 系统字体及微软雅黑回退，中文控制文案不使用斜体。正文14px，列表标题14px，日期28px（窄/短窗24px），辅助标签11px；时间使用等宽数字。列表标题最多两行，点标题编辑看全文；标签使用色点加名称，磁贴标题可换行。
...
待办为空时使用约96px的紧凑卡片。有事项时卡片随内容增高，不把剩余窗口拉成空白；事项多时整页滚动，日程紧接在待办下面。日程是一张连续外框：上部始终列出今日及逾期日程，下面明天、近期（后天至第7天）、更晚（第8天起）之间用细线分隔，保留展开收起；分组显示日期范围，组内按发生日期与时间排序，事项显示具体日期。未完成待办不论完成期限是否在未来都留在待办卡。主窗口仍由用户决定尺寸，不因空状态自动缩窗。

主窗口默认440×700 CSS px。「设置 → 通用 → 窗口宽度」提供标准440px与窄版340px，既有用户默认标准；选择即时调整并持久化，保持当前高度和距离最近的水平屏幕边缘，展开与收起共用所选宽度。独立悬浮入口当前从正式产品下线：启动与重启均不创建该窗口，主界面标题栏、设置和托盘不露出相关控制；既有窗口实现、图标资产、IPC 与用户保存的开关、位置、置顶和外观配置继续保留，且不因本次下线而改写。主窗口右下角星形按钮在窗口内打开 AI 对话，盖住下半截，高度固定 520px，矮窗口仍留出标题栏、日期和一条待办。边框使用与收起卡相同的暖色追光。独立对话窗口仍保留给定时和托盘入口，可在320×420至620×820之间拖动缩放。窄窗口内容内边距由24px降至17px；短窗口压缩顶部留白，列表和对话各自独立滚动，右下角 AI 星形入口保持可达。无边框标题条可拖动，交互元素显式排除拖动区域。关闭主界面不隐藏已打开的 AI；AI 已打开时再次调用只聚焦，手动移动后停止跟随，隐藏后重新打开才重新定位。
