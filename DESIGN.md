...
待办为空时使用约96px的紧凑卡片，不拉伸空边框填满窗口；有事项时仍让列表使用剩余空间。日程是一张连续外框：上部始终列出今日及逾期日程，下面明天、近期（后天至第7天）、更晚（第8天起）之间用细线分隔，保留展开收起；分组显示日期范围，组内按发生日期与时间排序，事项显示具体日期。未完成待办不论完成期限是否在未来都留在待办卡。主窗口仍由用户决定尺寸，不因空状态自动缩窗。
